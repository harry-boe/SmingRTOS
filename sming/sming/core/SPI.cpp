...
#define SPI_IDLE_EDGE 		(BIT(29))		// SPI_PIN register
...
SPIClass::SPIClass() {
#ifdef SPI_DEBUG
	debugf("SPIClass::SPIClass() construct HW SPI class");
#endif
	Serial.print("init SPI HW");
...
uint32_t SPIClass::getFrequency(int freq, int &pre, int clk) {
	...
	pre = divresult.quot;
	// check for div by zero -> exit with predevider 1 = _CPU_freq / 2
	if (pre == 0) {
		pre = 1;
		return _CPU_freq;
	}
...
void SPIClass::setFrequency(int freq) {
	...
	// there is no divider for target frequency > cpu_freq/2
	if (freq > _CPU_freq/2) {
		_SPISettings._speed = _CPU_freq/2;
		setClock(1, 2);
		return;
	}
